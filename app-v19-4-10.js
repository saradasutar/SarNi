...
const APP_BUILD_ID = '20260825-19410-loginheal1';
...
const MOBILE_DATA_CACHE_VERSION = '19.4.10-login-heal-1';
const state = {
...
async function loginWithAutomaticBrowserRecovery(username,password){
  ...
  try{
    return await api('login',payload,{timeoutMs:18000});
  }catch(error){
    ...
    return api('login',payload,{timeoutMs:30000});
  }
}
...
async function init(){
  try{
    if(window.__MYFINANCE_CACHE_HEAL_PENDING__)return;
...
console.info('MyFinance v19.4.10 loaded — automatic normal-browser login repair and overlap-free holdings summary enabled');
