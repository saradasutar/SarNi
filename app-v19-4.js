...
async function waitForAsyncResult(requestId,timeoutMs){
  ...
  while(Date.now()-started<timeoutMs){
    try{
      ...
      // A completed backend response (e.g. wrong password, rate limit, server error)
      // is terminal. Do not keep polling and turn it into a misleading timeout.
      if(err?.backendResult)throw err;
      lastError=err;
    }
  ...
  throw new Error('The backend request timed out. Backend v3.16.0 is reachable, but this action did not finish in time.');
}
...
async function login(event){
  ...
  try{
    const result=await api('login',{username:els.loginUsername.value.trim(),password:els.loginPassword.value},{timeoutMs:60000});
    ...
    if(!state.token){
      ...
      if(error.code==='INVALID_LOGIN')els.loginPassword.focus();
    }else{
...
console.info('MyFinance v19.4 loaded — terminal backend error handling + longer login timeout');
