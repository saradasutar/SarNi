...
const APP_BUILD_ID = '20260830-20000-premium1';
...
const MOBILE_DATA_CACHE_VERSION = '20.0.0-premium-command-centre-1';
const state = {
...
function applyDisplayPreferences(){
  let density='COMFORTABLE',theme='AUTO',sidebarCompact=false;
  try{
    density=localStorage.getItem(DISPLAY_DENSITY_KEY)||'COMFORTABLE';
    theme=localStorage.getItem(DISPLAY_THEME_KEY)||'AUTO';
...
function changeDisplayTheme(){
  try{localStorage.setItem(DISPLAY_THEME_KEY,els.displayThemeSelect?.value||'AUTO');}catch{}
  applyDisplayPreferences();
}
...
function bindEvents(){
  ...
  safeOn(els.holdingsMatrixCompareBtn,'click',toggleHoldingsMatrixCompare);
...
console.info('SarNi v20.0.0 loaded — premium responsive command centre ready');
