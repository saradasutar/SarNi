...
function renderHoldings(){
  ...
  els.holdingsBody.innerHTML=items.map(h=>{
    ...
    return `<tr class="holding-row" data-view-holding="${escapeHtml(h.id)}" tabindex="0" aria-label="View details for ${escapeHtml(h.assetName)}">
      ...
      <td>${formatCurrency(h.currentPrice)}<span class="price-note">${escapeHtml(h.priceSource||'Pending')}${h.priceDate?` · ${escapeHtml(detailDate(h.priceDate))}`:''}</span></td>
  ...
  appendCustomHeaders('HOLDINGS');applyStandardColumnSettings('HOLDINGS');installColumnResizers('holdings');applyStoredColumnWidths('holdings');bindTableScrollPersistence('holdings');restoreTableScroll('holdings');scheduleDashboardHScrollRefresh();
}
...
console.info('MyFinance v19.2.3 loaded — exact trade dates + realised/current/total P&L');
