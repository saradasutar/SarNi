...
async function loadDashboard(force=false){
  ...
  try{
    ...
    applyBootstrap(result.data);saveCache(result.data);setSyncStatus('','Up to date');
    if(force){
      ...
      if(nav){
        ...
        const msg=`MF NAV ${nav.updated}/${nav.tracked}${nav.remapped?` · ${nav.remapped} code remapped`:''} · MF performance ${perf?.updated??0}/${perf?.tracked??0}${issues?` · ${issues} need attention`:''}`;
...
function mfNavStatus(item){
  ...
  if(item.currentPrice==null||!Number.isFinite(Number(item.currentPrice))){
    return {cls:'stale',label:'NAV PENDING',title:'No usable mutual-fund NAV is currently available.'};
  }
...
console.info('MyFinance v19.2.5 loaded — active MF auto-refresh + NAV health + stale/manual indicators');
