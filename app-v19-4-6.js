...
const APP_VERSION = '19.4.6';
const EXPECTED_BACKEND_VERSION = '3.17.2';
...
const APP_BUILD_ID = '20260824-1946-fastlogin2';
...
const MOBILE_DATA_CACHE_VERSION = '19.4.6-fast-login-2';
const state = {
  ...
  try{
    ...
    if(data&&data.version){
      ...
      if(parts[0]<3||(parts[0]===3&&(parts[1]<17||(parts[1]===17&&(parts[2]||0)<2)))){
        showRuntimeWarning(`Backend v${state.backendVersion} is older than required v3.17.2. Deploy the included Code-v3.17.2.gs for fast login and holdings recovery.`);
      }
    }
  }catch(e){
...
function holdingAmountHealth(){
  ...
  const priced=priceable.filter(h=>
    !h.valueFromSnapshot&&((Number(h.currentValue)||0)>0 || (Number(h.currentPrice)||0)>0)
  ).length;
...
function setPortfolioStartupLoading(active,message='',error=false){
  ...
  if(els.portfolioStartupMessage){
    els.portfolioStartupMessage.textContent=message||(error
      ...
      :'Getting Holdings, prices and SIP data from Backend v3.17.2.');
  }
...
async function retryStartupPortfolioLoad(){
  if(!state.token)return;
  setPortfolioStartupLoading(true,'Retrying Backend v3.17.2…',false);
...
function applyPortfolioCore(data,{fromCache=false}={}){
  ...
  if(els.lastUpdatedText)els.lastUpdatedText.textContent=`${fromCache?'Showing last complete portfolio':'Holdings updated'} ${dateLabel(data.updatedAt)} · ${amountHealth.displayed}/${amountHealth.priceable} values visible`;
  return true;
}

async function loadPortfolioCore({startup=false,silent=false}={}){
  ...
  try{
    const result=await api('bootstrapCore',{}, {retry:true,timeoutMs:45000});
    if(!isUsableDashboardData(result?.data)){
...
function applyBootstrap(data,fromCache=false){
  ...
  const instantNote=snapshotCount?' · live update running':'';
...
console.info('MyFinance v19.4.6 loaded — fast login and empty-holdings recovery enabled');
