...
const APP_VERSION = '19.4.9';
const EXPECTED_BACKEND_VERSION = '3.17.4';
...
const APP_BUILD_ID = '20260825-1949-gpfsummary2';
...
function clearTransientBrowserStateForUpdate(){
  try{
    ...
    // Promote the last correct device values before removing old volatile
    // portfolio caches. The stable snapshot survives future cache repairs.
    preserveLastGoodValuesFromLegacyCache();

    // Preserve long-term UI preferences and saved username.
    // Clear only session/runtime data that can make a normal browser behave
    // differently from a clean Incognito window.
    Object.keys(localStorage).forEach(key=>{
      if(
        key==='portfolio_token' ||
        key==='portfolio_username' ||
        key===LAST_ACTIVITY_KEY ||
        key===LAST_HIDDEN_KEY ||
        key.startsWith('portfolio_cache_')
      ){
        localStorage.removeItem(key);
      }
    });
    localStorage.setItem(APP_BUILD_STORAGE_KEY,APP_BUILD_ID);
    localStorage.setItem('myfinance_browser_repaired_notice','1');
    return true;
  }catch(e){return false;}
}
const BROWSER_STORAGE_REPAIRED_ON_LOAD = clearTransientBrowserStateForUpdate();

const MOBILE_DATA_CACHE_VERSION = '19.4.9-gpf-summary-2';
const state = {
...
async function waitForAsyncResult(requestId,timeoutMs){
  ...
  throw new Error('The backend did not return a result. Confirm Backend v3.17.4 is deployed and config.js uses the same /exec URL.');
}
function apiViaAsyncPost(action,payload={},options={}){
  ...
  return new Promise((resolve,reject)=>{
    ...
    const onMessage=event=>{
      ...
      if(!trustedGoogleOrigin(event.origin)||event.source!==frame.contentWindow)return;
    ...
    // Backend v3.17.4 replies directly to the hidden frame. Polling starts only
    // as a compatibility fallback, avoiding a burst of competing executions.
    const fallbackDelay=Math.min(1800,Math.max(700,Math.round(timeoutMs*.06)));
  ...
  try{
    ...
    if(data&&data.version){
      ...
      if(parts[0]<3||(parts[0]===3&&(parts[1]<17||(parts[1]===17&&(parts[2]||0)<2)))){
        showRuntimeWarning(`Backend v${state.backendVersion} is older than required v3.17.4. Deploy the included Code-v3.17.4.gs for smooth login and GPF holdings.`);
      }
    }
  }catch(e){
...
async function login(event){
  ...
  try{
    const result=await api('login',{username:els.loginUsername.value.trim(),password:els.loginPassword.value,fastLogin:true},{timeoutMs:35000});
...
async function manualRepairBrowser(){
  ...
  const next=`${location.pathname}?v=${encodeURIComponent(APP_VERSION)}&repair=${Date.now()}`;
...
async function retryStartupPortfolioLoad(){
  if(!state.token)return;
  setPortfolioStartupLoading(true,'Retrying Backend v3.17.4…',false);
...
function applyBootstrap(data,fromCache=false){
  ...
  updateVersionLabels();
  state.user=data.user||state.user;state.holdings=mergeGoodValueSnapshot(data.holdings);state.transactions=Array.isArray(data.transactions)?data.transactions:[];state.watchlist=Array.isArray(data.watchlist)?data.watchlist:[];state.sipPlans=Array.isArray(data.sipPlans)?data.sipPlans:[];state.sipEvents=Array.isArray(data.sipEvents)?data.sipEvents:[];state.expenses=Array.isArray(data.expenses)?data.expenses:[];state.expensePlans=Array.isArray(data.expensePlans)?data.expensePlans:[];state.recurringExpenses=Array.isArray(data.recurringExpenses)?data.recurringExpenses:[];state.expenseCategories=Array.isArray(data.expenseCategories)?data.expenseCategories:[];state.customColumns=Array.isArray(data.customColumns)?data.customColumns:[];state.customValues=Array.isArray(data.customValues)?data.customValues:[];state.stickyNotes=Array.isArray(data.stickyNotes)?data.stickyNotes:[];state.lifeQuotes=Array.isArray(data.lifeQuotes)?data.lifeQuotes:[];state.diary=Array.isArray(data.diary)?data.diary:[];state.monthlyDiary=Array.isArray(data.monthlyDiary)?data.monthlyDiary:[];state.monthStatus=Array.isArray(data.monthStatus)?data.monthStatus:[];state.owners=Array.isArray(data.owners)?data.owners:[];if(Array.isArray(data.users))state.users=data.users;
  persistGoodValueSnapshot(data.updatedAt||'');
  protectInitialHoldingView();
...
const EXPENSE_TABLE_COLUMNS={
  ACTUAL:[['amount','Amount'],['date','Dt Spent'],['paidTo','Paid To / Name'],['account','Debited From'],['reason','Reason'],['category','Category of Exp'],['notes','Notes to Remember'],['actions','Actions']],
  PLANNED:[['amount','Planned Amount'],['date','Planned Date'],['paidTo','Paid To / Name'],['account','Debited From'],['reason','Reason'],['category','Category'],['notes','Notes'],['status','Status'],['actions','Actions']],
  REGULAR:[['name','Regular Payment'],['amount','Amount'],['account','Debited From'],['category','Category'],['frequency','Frequency'],['date','Next Due'],['reason','Reason'],['notes','Notes'],['status','Status'],['actions','Actions']]
};
...
function expenseFilterRecords(view=expenseCurrentView()){
  ...
  const cat=els.expenseCategoryFilter?.value||'ALL';const acct=els.expenseAccountFilter?.value||'ALL';
  let rows=expenseRecords(view).filter(item=>{
    const date=expenseDateField(item,view)||'';
    const hay=`${item.amount||''} ${item.paidTo||''} ${item.name||''} ${item.debitedFrom||''} ${item.reason||''} ${item.category||''} ${item.notes||''} ${item.status||''}`.toLowerCase();
    return (!q||hay.includes(q))&&(!from||date>=from)&&(!to||date<=to)&&(cat==='ALL'||item.category===cat)&&(acct==='ALL'||item.debitedFrom===acct);
  });
...
function populateExpenseFilters(){
  const categories=[...new Set(state.expenseCategories.map(c=>c.name).filter(Boolean))].sort();
  const accounts=[...new Set([...state.expenses,...state.expensePlans,...state.recurringExpenses].map(x=>x.debitedFrom).filter(Boolean))].sort();
...
function renderActualExpenses(rows){
  ...
  els.actualExpenseBody.innerHTML=rows.map(x=>`<tr>
    <td class="expense-amount">${formatCurrency(x.amount)}</td><td>${expenseDateDisplay(x.dateSpent)}</td><td>${escapeHtml(x.paidTo||'—')}</td>
    <td>${escapeHtml(x.debitedFrom||'—')}</td><td class="expense-reason">${escapeHtml(x.reason||'—')}</td><td>${expenseCategoryBadge(x.category)}</td>
    <td class="expense-notes">${escapeHtml(x.notes||'—')}</td><td class="expense-actions"><button class="small-button" data-edit-expense="${escapeHtml(x.id)}">Edit</button><button class="small-button danger" data-delete-expense="${escapeHtml(x.id)}">Delete</button></td>
  </tr>`).join('');
}
function renderPlannedExpenses(rows){
  ...
  els.plannedExpenseBody.innerHTML=rows.map(x=>{const planned=String(x.status||'PLANNED').toUpperCase()==='PLANNED';return `<tr>
    <td class="expense-amount">${formatCurrency(x.amount)}</td><td>${expenseDateDisplay(x.plannedDate)}</td><td>${escapeHtml(x.paidTo||'—')}</td>
    <td>${escapeHtml(x.debitedFrom||'—')}</td><td class="expense-reason">${escapeHtml(x.reason||'—')}</td><td>${expenseCategoryBadge(x.category)}</td><td class="expense-notes">${escapeHtml(x.notes||'—')}</td><td>${expenseStatusBadge(x.status)}</td>
    <td class="expense-actions">${planned?`<button class="small-button success" data-mark-plan-paid="${escapeHtml(x.id)}">Mark paid</button>`:''}<button class="small-button" data-edit-plan="${escapeHtml(x.id)}">Edit</button><button class="small-button danger" data-delete-plan="${escapeHtml(x.id)}">Delete</button></td>
  </tr>`;}).join('');
}
function renderRegularExpenses(rows){
  ...
  els.regularExpenseBody.innerHTML=rows.map(x=>{const active=String(x.status||'ACTIVE').toUpperCase()==='ACTIVE';const toggle=active?'PAUSED':'ACTIVE';return `<tr>
    <td class="expense-reason"><strong>${escapeHtml(x.name||'—')}</strong><small>${escapeHtml(x.paidTo||'')}</small></td><td class="expense-amount">${formatCurrency(x.amount)}</td><td>${escapeHtml(x.debitedFrom||'—')}</td><td>${expenseCategoryBadge(x.category)}</td>
    ...
    <td class="expense-actions">${active?`<button class="small-button success" data-mark-regular-paid="${escapeHtml(x.id)}">Mark paid</button>`:''}<button class="small-button" data-toggle-regular="${escapeHtml(x.id)}" data-next-status="${toggle}">${active?'Pause':'Resume'}</button><button class="small-button" data-edit-regular="${escapeHtml(x.id)}">Edit</button><button class="small-button danger" data-delete-regular="${escapeHtml(x.id)}">Delete</button></td>
  </tr>`;}).join('');
}
function renderExpenseCategories(){
  ...
  els.expenseCategoryGrid.innerHTML=rows.map(c=>{const used=state.expenses.filter(x=>x.category===c.name).length+state.expensePlans.filter(x=>x.category===c.name).length+state.recurringExpenses.filter(x=>x.category===c.name).length;return `<article class="expense-category-card ${String(c.color||'BLUE').toLowerCase()}">
    <div><span class="expense-category-dot"></span><strong>${escapeHtml(c.name)}</strong><small>${used} record${used===1?'':'s'}</small></div><div><button class="small-button" data-edit-expense-category="${escapeHtml(c.id)}">Edit</button><button class="small-button danger" data-delete-expense-category="${escapeHtml(c.id)}">Delete</button></div>
  </article>`;}).join('');
}
function renderExpenditure(){
  ...
  if(view==='CATEGORIES'){renderExpenseCategories();return;}
...
function resetExpenseFilters(){
  if(els.expenseSearch)els.expenseSearch.value='';if(els.expenseFromDate)els.expenseFromDate.value='';if(els.expenseToDate)els.expenseToDate.value='';if(els.expenseCategoryFilter)els.expenseCategoryFilter.value='ALL';if(els.expenseAccountFilter)els.expenseAccountFilter.value='ALL';if(els.expenseSort)els.expenseSort.value='DATE_DESC';renderExpenditure();
}
...
function fillExpenseCategorySelect(value=''){
  populateExpenseFilters();if(els.expenseCategory&&value&&[...els.expenseCategory.options].some(o=>o.value===value))els.expenseCategory.value=value;
}
...
function openExpenseCategoryEditor(item=null){
  els.expenseCategoryForm?.reset();els.expenseCategoryId.value=item?.id||'';els.expenseCategoryModalTitle.textContent=item?'Edit category':'Add category';els.expenseCategoryName.value=item?.name||'';els.expenseCategoryColor.value=item?.color||'BLUE';openModal('expenseCategoryModal');
}
async function saveExpenseCategory(event){
  event.preventDefault();setBusy(els.saveExpenseCategoryBtn,true,'Saving…');try{const r=await api('saveExpenseCategory',{category:{id:els.expenseCategoryId.value.trim(),name:els.expenseCategoryName.value.trim(),color:els.expenseCategoryColor.value}});applyBootstrap(r.data);saveCache(r.data);closeModals();setExpenseView('CATEGORIES');toast('Expense category saved.','success');}catch(e){toast(e.message,'error');}finally{setBusy(els.saveExpenseCategoryBtn,false);}
}
...
function renderSummary(){
  ...
  els.sumCurrent.textContent=s.valuesComplete?formatCurrency(s.current,true):'Updating…';
  els.sumGain.textContent=s.valuesComplete?formatCurrency(s.gain,true):'—';
    ...
    if(d.key==='GPF'){
      ...
      return `<button type="button" class="type-summary-card ${active}" data-asset-view="${d.key}">
        ...
        <div class="type-summary-values">
          <div><span>Present balance</span><b>${formatCurrency(s.current,true)}</b></div>
          <div><span>Monthly payment</span><b>${formatCurrency(monthly,true)}</b></div>
          <div><span>Est. interest</span><b class="positive">${formatCurrency(interest,true)}</b></div>
          <div><span>Projected balance</span><b>${formatCurrency(projected,true)}</b></div>
        </div>
      </button>`;
    }
    return `<button type="button" class="type-summary-card ${active}" data-asset-view="${d.key}">
      ...
      <div class="type-summary-values">
        ...
        <div><span>Current</span><b>${s.valuesComplete?formatCurrency(s.current,true):'Updating…'}</b></div>
        <div><span>Gain / loss</span><b class="${s.valuesComplete?pnlClass(s.gain):'neutral'}">${s.valuesComplete?formatCurrency(s.gain,true):'—'}</b></div>
...
function setHoldingValueGrowth(prefix,items){
  ...
  if(growthEl){
    const gain=v.valuesComplete?`${v.gain>=0?'+':'-'}${formatCurrency(Math.abs(v.gain),true)}`:'Updating prices…';
    const pct=v.valuesComplete?`${v.returnPct>=0?'+':''}${v.returnPct.toFixed(2)}%`:'';
    growthEl.textContent=v.valuesComplete?`${gain} · ${pct}`:gain;
...
function setGpfHoldingSummary(prefix,items){
  ...
  if(present)present.textContent=formatCurrency(total.present,true);
  if(monthly)monthly.textContent=formatCurrency(total.monthly,true);
  if(projected)projected.textContent=formatCurrency(total.projected,true);
  if(interest)interest.textContent=`${formatCurrency(total.interest,true)} estimated interest`;
  const card=present?.closest('.holdings-mini-card.gpf');
  if(card){
    card.classList.toggle('gpf-empty',total.count===0);
    card.title=total.count?`${total.count} GPF holding${total.count===1?'':'s'} included in this summary`:'No GPF holding added for this investor';
  }
}
...
function renderHoldings(){
  ...
  els.holdingsBody.innerHTML=items.map(h=>{
    ...
    const mainRow=`<tr class="holding-row" data-view-holding="${escapeHtml(h.id)}" tabindex="0" aria-label="View details for ${escapeHtml(h.assetName)}">
      ...
      <td>${isGpf?'—':formatNumber(h.units)}</td><td>${isGpf?'—':formatCurrency(avg)}</td><td>${formatCurrency(h.investedAmount)}</td>
...
console.info('MyFinance v19.4.9 loaded — smooth login, explicit GPF holdings summary and professional diary enabled');
