...
const APP_BUILD_ID = '20260824-1943-mobilelogin2';
...
const MOBILE_DATA_CACHE_VERSION = '19.4.3-mobile-login-2';
const state = {
...
async function login(event){
  ...
  try{
    ...
    // potentially long price refresh runs. The login response is already the
    // authoritative dashboard snapshot; repair missing values in the background.
...
function applyBootstrap(data,fromCache=false){
  ...
  updateVersionLabels();
  state.user=data.user||state.user;state.holdings=Array.isArray(data.holdings)?data.holdings:[];state.transactions=Array.isArray(data.transactions)?data.transactions:[];state.watchlist=Array.isArray(data.watchlist)?data.watchlist:[];state.sipPlans=Array.isArray(data.sipPlans)?data.sipPlans:[];state.sipEvents=Array.isArray(data.sipEvents)?data.sipEvents:[];state.expenses=Array.isArray(data.expenses)?data.expenses:[];state.expensePlans=Array.isArray(data.expensePlans)?data.expensePlans:[];state.recurringExpenses=Array.isArray(data.recurringExpenses)?data.recurringExpenses:[];state.expenseCategories=Array.isArray(data.expenseCategories)?data.expenseCategories:[];state.customColumns=Array.isArray(data.customColumns)?data.customColumns:[];state.customValues=Array.isArray(data.customValues)?data.customValues:[];state.stickyNotes=Array.isArray(data.stickyNotes)?data.stickyNotes:[];state.lifeQuotes=Array.isArray(data.lifeQuotes)?data.lifeQuotes:[];state.diary=Array.isArray(data.diary)?data.diary:[];state.monthlyDiary=Array.isArray(data.monthlyDiary)?data.monthlyDiary:[];state.monthStatus=Array.isArray(data.monthStatus)?data.monthStatus:[];state.owners=Array.isArray(data.owners)?data.owners:[];if(Array.isArray(data.users))state.users=data.users;
...
console.info('MyFinance v19.4.3 loaded — mobile login fast-path enabled');
