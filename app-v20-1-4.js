...
const APP_BUILD_ID = '20260901-20104-regularpayments1';
...
const MOBILE_DATA_CACHE_VERSION = '20.1.4-premium-command-centre-1';
const state = {
...
const REGULAR_PAYMENT_EXAMPLES = {
  axis: {name:'Credit Card - Axis',category:'Credit Card',subcategory:'Axis',paidTo:'Axis Bank'},
  icici: {name:'Credit Card - ICICI',category:'Credit Card',subcategory:'ICICI',paidTo:'ICICI Bank'},
  hdfc: {name:'Credit Card - HDFC',category:'Credit Card',subcategory:'HDFC',paidTo:'HDFC Bank'},
...
function openRegularPaymentExample(key){
  ...
  const existing=state.recurringExpenses.find(x=>String(x.name||'').toLowerCase()===preset.name.toLowerCase()||
...
function renderRegularBillQuickList(rows){
  ...
  list.innerHTML=rows.map(item=>{
    ...
    return `<article class="regular-bill-card ${active?'':'is-paused'}">
      ...
      <div class="regular-bill-context"><span>Next due: <strong>${expenseDateDisplay(item.nextDueDate)}</strong></span><span>Planning estimate: ${formatCurrency(item.amount)}</span><span>${latest?`Last paid: ${formatCurrency(latest.amount)} · ${expenseDateDisplay(latest.dateSpent)}`:'No payment recorded yet'}</span></div>
      <form data-regular-bill="${escapeHtml(item.id)}" data-draft-key="${escapeHtml(key)}" data-due-date="${escapeHtml(item.nextDueDate||'')}">
        <fieldset class="regular-bill-fields" ${blocked?'disabled':''}>
          <label>Actual amount paid (₹)<input type="number" min="0.01" step="0.01" required inputmode="decimal" data-regular-field="amount" value="${escapeHtml(draft.amount||'')}" placeholder="Enter this payment’s amount" aria-label="${escapeHtml(name)} actual amount paid"></label>
          ...
          <label class="regular-bill-note">Note / payment reference <small>(optional)</small><input type="text" maxlength="600" data-regular-field="notes" value="${escapeHtml(draft.notes||'')}" placeholder="Bill month, receipt or reference number"></label>
        </fieldset>
      </form>
...
function bindRegularBillQuickEntry(){
  $('holdingSummaryEndBtn')?.addEventListener('click',jumpToHoldingSummaryEnd);
...
function updateHoldingSummaryNavigator(){
  const wrap=document.querySelector('.holding-matrix-wrap'),nav=els.holdingSummaryNavigator;if(!wrap||!nav)return;
  const compare=wrap.classList.contains('is-compare-all'),max=Math.max(0,wrap.scrollWidth-wrap.clientWidth),pct=max?Math.round(wrap.scrollLeft/max*100):0;
  nav.classList.toggle('hidden',!compare&&max<=1);
  if(els.holdingSummaryScrollRange){els.holdingSummaryScrollRange.value=String(pct);els.holdingSummaryScrollRange.disabled=max<=1;}
  if(els.holdingSummaryScrollLabel)els.holdingSummaryScrollLabel.textContent=`${pct}%`;
  if(els.holdingSummaryLeftBtn)els.holdingSummaryLeftBtn.disabled=wrap.scrollLeft<=1;
  if(els.holdingSummaryRightBtn)els.holdingSummaryRightBtn.disabled=wrap.scrollLeft>=max-1;
  const jump=$('holdingSummaryEndBtn');if(jump){jump.hidden=!compare||max<=1;jump.disabled=wrap.scrollLeft>=max-1;}
  const hint=$('holdingSummaryScrollHint');if(hint)hint.textContent=max<=1?'All summary columns are visible':wrap.scrollLeft>=max-1?'Rightmost columns visible · move left to return':'More columns to the right →';
}
...
function jumpToHoldingSummaryEnd(){setHoldingsMatrixCompare(true);requestAnimationFrame(()=>setHoldingSummaryScrollPercent(100));}
...
console.info('SarNi v20.1.4 loaded — premium responsive command centre ready');
