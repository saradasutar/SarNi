...
const APP_VERSION = '19.4.5';
const EXPECTED_BACKEND_VERSION = '3.17.1';
...
const MOBILE_DATA_CACHE_VERSION = '19.4.5-instant-values-1';
const state = {
...
function startQuickValueSync(){
  ...
  state.quickValueSyncTimer=setInterval(()=>{
    if(!state.token||document.hidden||state.syncing)return;
    const age=Date.now()-Number(state.lastServerSyncAt||0);
    if(age<45000)return;
    loadDashboard(false,{startup:false}).catch(error=>console.warn('Quick value sync failed:',error));
  },QUICK_VALUE_SYNC_MS);
}
...
async function loadBackendVersion(){
  ...
  try{
    ...
    if(data&&data.version){
      ...
      if(parts[0]<3||(parts[0]===3&&parts[1]<17)){
        showRuntimeWarning(`Backend v${state.backendVersion} is older than required v3.17.0. Update the Apps Script backend before using Expenditure.`);
      }
    }
  }catch(e){
...
async function login(event){
  ...
  try{
    const result=await api('login',{username:els.loginUsername.value.trim(),password:els.loginPassword.value},{timeoutMs:isMobileViewport()?120000:90000});
    if(!result?.token||!result?.user?.username||!result?.data){
      throw new Error('Login completed without dashboard data. Please try once more.');
    }
    ...
    setPortfolioStartupLoading(true,'Loading your portfolio after sign-in…',false);
    applyBootstrap(result.data);
    saveCache(result.data);
    state.lastServerSyncAt=Date.now();
    ...
    // Do not keep a phone on the login screen while a second bootstrap or a
    // potentially long price refresh runs. Show the login snapshot immediately,
    // then confirm the shared backend values in the background on every device.
    const signedInToken=state.token;
    setTimeout(()=>{
      if(!state.token||state.token!==signedInToken)return;
      repairHoldingAmounts({allowPriceRefresh:true,skipBootstrap:true})
        .then(repaired=>{
          if(!state.token||state.token!==signedInToken)return;
          if(repaired){
            renderAll();
            ...
            setSyncStatus('','Signed in · prices can be refreshed later');
          }
        })
        .catch(error=>{
          console.warn('Background holding value repair failed:',error);
          if(state.token===signedInToken)setSyncStatus('','Signed in · prices can be refreshed later');
        });
    },100);
  }
...
function setPortfolioStartupLoading(active,message='',error=false){
  ...
  if(els.portfolioStartupMessage){
    els.portfolioStartupMessage.textContent=message||(error
      ...
      :'Getting Holdings, prices and SIP data from Backend v3.17.0.');
  }
...
async function retryStartupPortfolioLoad(){
  if(!state.token)return;
  setPortfolioStartupLoading(true,'Retrying Backend v3.17.0…',false);
...
async function refreshMobileFromBackend(reason='resume'){
  ...
  try{
    await loadDashboard(false,{startup:false});
...
async function init(){
  try{
    ...
    if(state.token){
      showApp();
      setPortfolioStartupLoading(true,'Restoring your saved session and loading the latest portfolio…',false);
      await loadDashboard(false,{startup:true});
      restoreHoldingsSummaryState();
      resetAutoRefreshClock();
    }
...
console.info('MyFinance v19.4.5 loaded — instant values and fast shared sync enabled');
