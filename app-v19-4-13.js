...
const APP_VERSION = '19.4.13';
const EXPECTED_BACKEND_VERSION = '3.17.5';
...
const APP_BUILD_ID = '20260825-19413-newexec1';
...
const MOBILE_DATA_CACHE_VERSION = '19.4.13-new-exec-1';
const state = {
...
async function waitForAsyncResult(requestId,timeoutMs){
  ...
  throw new Error('The backend did not return a result. Confirm Backend v3.17.5 is deployed and config.js uses the same /exec URL.');
}
function apiViaAsyncPost(action,payload={},options={}){
  ...
  return new Promise((resolve,reject)=>{
    ...
    // Backend v3.17.5 replies directly to the hidden frame. Polling starts only
  ...
  try{
    ...
    if(data&&data.version){
      ...
      if(parts[0]<3||(parts[0]===3&&(parts[1]<17||(parts[1]===17&&(parts[2]||0)<2)))){
        showRuntimeWarning(`Backend v${state.backendVersion} is older than required v3.17.5. Deploy the included Code-v3.17.5.gs for secure same-origin login and GPF holdings.`);
      }
    }
  }catch(e){
...
async function retryStartupPortfolioLoad(){
  if(!state.token)return;
  setPortfolioStartupLoading(true,'Retrying Backend v3.17.5…',false);
...
console.info('MyFinance v19.4.13 loaded — corrected Apps Script deployment and overlap-free holdings summary enabled');
