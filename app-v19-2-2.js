...
function setHoldingValueGrowth(prefix,items){
  ...
  if(growthEl){
    ...
    growthEl.classList.add(v.gain>0?'positive':v.gain<0?'negative':'neutral');
  }
}
...
const STANDARD_COLUMN_DEFS={
  HOLDINGS:[
    ...
    {key:'tradeDates',label:'Purchase / Sale Date',index:2},
    {key:'units',label:'Qty / Units',index:3},{key:'avgBuy',label:'Avg Buy',index:4},{key:'invested',label:'Invested',index:5},
    {key:'currentPrice',label:'Current Price / NAV',index:6},{key:'currentValue',label:'Current Value',index:7},
    {key:'gainLoss',label:'Gain / Loss',index:8},{key:'gainPct',label:'Gain %',index:9},{key:'xirr',label:'XIRR',index:10},
    {key:'d1',label:'1D',index:11},{key:'w1',label:'1W',index:12},{key:'m1',label:'1M',index:13},{key:'m6',label:'6M',index:14},
    {key:'y1',label:'1Y',index:15},{key:'y3',label:'3Y',index:16},{key:'y5',label:'5Y',index:17},{key:'y10',label:'10Y',index:18},
    {key:'note',label:'Personal Note',index:19},{key:'actions',label:'Actions',index:20,locked:true}
  ],
  WATCHLIST:[
...
function holdingTradeDatesHtml(h){
  const s=h?.transactionStats||{};
  const purchase=s.firstPurchaseDate||h.buyDate||'';
  const latestPurchase=s.latestPurchaseDate||'';
  const sale=s.latestSaleDate||'';
  const purchaseLabel=purchase?detailDate(purchase):'—';
  const saleLabel=sale?detailDate(sale):'—';
  const extra=latestPurchase&&latestPurchase!==purchase?` title="Latest purchase: ${escapeHtml(detailDate(latestPurchase))}"`:'';
  return `<div class="holding-trade-dates"${extra}>
    <span class="holding-trade-date purchase"><b>BUY</b><strong>${escapeHtml(purchaseLabel)}</strong></span>
    <span class="holding-trade-date sale ${sale?'has-sale':'no-sale'}"><b>SALE</b><strong>${escapeHtml(saleLabel)}</strong></span>
  </div>`;
}
...
function renderWatchlist(){
  ...
  els.watchBody.innerHTML=items.map(x=>{
    ...
    return `<tr class="watch-row" data-view-watch="${escapeHtml(x.id)}" tabindex="0" aria-label="View watchlist details for ${escapeHtml(x.assetName)}">
      ...
      <td>${formatCurrency(x.currentPrice)}<span class="price-note">${escapeHtml(x.priceSource||'Pending')}</span></td>
...
function printHoldingsView(){
  ...
  const rows=items.map(h=>{const p=h.performance||{},s=h.transactionStats||{},purchase=s.firstPurchaseDate||h.buyDate||'',sale=s.latestSaleDate||'';const customCells=custom.map(c=>`<td>${printEscape(formatCustomValuePlain(c,customValueFor('HOLDINGS',h.id,c.columnKey)))}</td>`).join('');return `<tr><td>${printEscape(shortOwner(h.owner))}</td><td>${printEscape(h.assetName)}<br><small>${printEscape(h.type)} ${printEscape(h.code||'')}</small></td><td><span class="pos">BUY ${printEscape(purchase?detailDate(purchase):'—')}</span><br><span class="${sale?'neg':''}">SALE ${printEscape(sale?detailDate(sale):'—')}</span></td><td class="num">${printEscape(formatNumber(h.units))}</td><td class="num">${printEscape(formatCurrency(h.investedAmount))}</td><td class="num">${printEscape(formatCurrency(h.currentValue))}</td><td class="num ${Number(h.gainLoss)>=0?'pos':'neg'}">${printEscape(formatCurrency(h.gainLoss))}</td><td class="num ${Number(h.returnPct)>=0?'pos':'neg'}">${printEscape(formatPercent(h.returnPct))}</td><td class="num">${printEscape(formatPercent(h.xirr))}</td><td class="num">${printEscape(formatPercent(p.m1))}</td><td class="num">${printEscape(formatPercent(p.y1))}</td><td class="num">${printEscape(formatPercent(p.y3))}</td><td>${printEscape(h.notes||'')}</td>${customCells}</tr>`;}).join('');
  const map=[0,1,2,3,5,7,8,9,10,13,15,16,19,...custom.map((c,i)=>20+i)];
...
console.info('MyFinance v19.2.2 loaded — purchase/sale date column + keyboard smooth scroll + column manager');
