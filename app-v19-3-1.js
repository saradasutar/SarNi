...
const EXPECTED_BACKEND_VERSION = '3.16.0';
const MOBILE_BREAKPOINT = 860;
const MOBILE_DATA_CACHE_VERSION = '19.3.0-sip-planner-1';
const state = {
  ...
  try{
    ...
    if(data&&data.version){
      ...
      if(parts[0]<3||(parts[0]===3&&parts[1]<16)){
        showRuntimeWarning(`Backend v${state.backendVersion} is older than required v3.16.0. Update the Apps Script backend before using SIP Planner.`);
      }
    }
  }catch(e){
...
function setPortfolioStartupLoading(active,message='',error=false){
  ...
  if(els.portfolioStartupMessage){
    els.portfolioStartupMessage.textContent=message||(error
      ...
      :'Getting Holdings, prices and SIP data from Backend v3.16.0.');
  }
...
async function retryStartupPortfolioLoad(){
  ...
  setPortfolioStartupLoading(true,'Retrying Backend v3.16.0…',false);
...
function applyBootstrap(data,fromCache=false){
  ...
  updateVersionLabels();
  state.user=data.user||state.user;state.holdings=Array.isArray(data.holdings)?data.holdings:[];state.transactions=Array.isArray(data.transactions)?data.transactions:[];state.watchlist=Array.isArray(data.watchlist)?data.watchlist:[];state.sipPlans=Array.isArray(data.sipPlans)?data.sipPlans:[];state.sipEvents=Array.isArray(data.sipEvents)?data.sipEvents:[];state.customColumns=Array.isArray(data.customColumns)?data.customColumns:[];state.customValues=Array.isArray(data.customValues)?data.customValues:[];state.stickyNotes=Array.isArray(data.stickyNotes)?data.stickyNotes:[];state.lifeQuotes=Array.isArray(data.lifeQuotes)?data.lifeQuotes:[];state.diary=Array.isArray(data.diary)?data.diary:[];state.monthlyDiary=Array.isArray(data.monthlyDiary)?data.monthlyDiary:[];state.monthStatus=Array.isArray(data.monthStatus)?data.monthStatus:[];state.owners=Array.isArray(data.owners)?data.owners:[];if(Array.isArray(data.users))state.users=data.users;
...
function renderAll(){refreshAssetViewControls();renderSummary();renderGrowthDashboard();renderTypeSummary();renderAllocation();renderInvestorSummary();renderTopHoldings();renderHoldings();renderTransactions();renderWatchlist();renderSipPlanner();renderLifeQuote(true);renderStickyNotes();renderPersonalHome();refreshMonthlyYearFilter();renderDiary();renderMonthlyDiary();setOverviewMode(state.overviewMode);if(state.user?.role==='ADMIN')renderUsers();}
...
function switchSection(section){
  ...
  document.body.classList.remove('section-overview','section-holdings','section-transactions','section-watchlist','section-sip','section-diary','section-users');
  document.body.classList.add(`section-${section}`);
  const titles={overview:state.overviewMode==='INVESTMENT'?'Investment overview':'My dashboard',holdings:'Holdings & performance',transactions:'Transactions',watchlist:'Watchlist',sip:'SIP Planner & Future Investment',diary:'Diary',users:'User administration'};
  if(els.pageTitle)els.pageTitle.textContent=titles[section]||'My Finance';
  ['overview','holdings','transactions','watchlist','sip','diary','users'].forEach(name=>{
...
console.info('MyFinance v19.3.1 loaded — SIP Planner + startup sync protection');
