...
const APP_VERSION = '19.4.7';
const EXPECTED_BACKEND_VERSION = '3.17.3';
...
const APP_BUILD_ID = '20260824-1947-sharedrecovery1';
...
const MOBILE_DATA_CACHE_VERSION = '19.4.7-shared-recovery-1';
const state = {
  ...
  try{
    ...
    if(data&&data.version){
      ...
      if(parts[0]<3||(parts[0]===3&&(parts[1]<17||(parts[1]===17&&(parts[2]||0)<2)))){
        showRuntimeWarning(`Backend v${state.backendVersion} is older than required v3.17.3. Deploy the included Code-v3.17.3.gs for shared holdings recovery.`);
      }
    }
  }catch(e){
...
async function login(event){
  ...
  try{
    const result=await api('login',{username:els.loginUsername.value.trim(),password:els.loginPassword.value,fastLogin:true},{timeoutMs:60000});
    ...
    if(isUsableDashboardData(result.data)){
      ...
      setPortfolioStartupLoading(true,'Signed in · loading your holdings first…',false);
    ...
    const signedInToken=state.token;
    if(!isUsableDashboardData(result.data)){
      const coreReady=await loadPortfolioCore({startup:!ready});
      ready=ready||coreReady;
    }
    if(!state.token||state.token!==signedInToken)return;
    if(ready){
      setPortfolioStartupLoading(false);
      setSyncStatus('','Holdings ready · full details updating');
      setTimeout(()=>{
        if(state.token===signedInToken)loadDashboard(false,{startup:false});
      },150);
    }else{
      await loadDashboard(false,{startup:true});
    }
  }
...
function setPortfolioStartupLoading(active,message='',error=false){
  ...
  if(els.portfolioStartupMessage){
    els.portfolioStartupMessage.textContent=message||(error
      ...
      :'Getting holdings and verified values from Backend v3.17.3.');
  }
...
async function retryStartupPortfolioLoad(){
  if(!state.token)return;
  setPortfolioStartupLoading(true,'Retrying Backend v3.17.3…',false);
...
async function loadPortfolioCore({startup=false,silent=false,forceShared=false}={}){
  ...
  try{
    ...
    let result=await api('bootstrapCore',payload, {retry:true,timeoutMs:45000});
    const firstHealth=holdingRowsHealth(result?.data?.holdings);
    const needsShared=!isUsableDashboardData(result?.data)||(firstHealth.priceable>0&&firstHealth.invested>0&&firstHealth.displayed<firstHealth.priceable);
    if(needsShared&&!forceShared){
      if(!silent)setSyncStatus('syncing','Recovering verified holding values…');
      result=await api('bootstrapCore',{forceSharedRecovery:true,recoveryNonce:Date.now()}, {retry:true,timeoutMs:45000});
    }
    if(!isUsableDashboardData(result?.data)){
...
async function loadDashboard(force=false,{startup=false}={}){
  ...
  try{
    const result=await api(force?'refreshPrices':'bootstrap',{}, {retry:!force,timeoutMs:force?120000:90000});
    ...
    if(!force&&holdingAmountsMissing()&&!state.mobileAmountRepairTried){
      await repairHoldingAmounts({allowPriceRefresh:true,skipBootstrap:true});
    }
...
async function refreshMobileFromBackend(reason='resume'){
  ...
  try{
    ...
    if(!coreLoaded)await loadDashboard(false,{startup:false});
    if(holdingAmountsMissing()){
      await repairHoldingAmounts({allowPriceRefresh:!state.mobileAmountRepairTried,skipBootstrap:true});
    }
...
async function init(){
  try{
    ...
    if(state.token){
      ...
      else setPortfolioStartupLoading(true,'Restoring your session · loading holdings first…',false);
      const coreReady=await loadPortfolioCore({startup:!saved});
      if(!coreReady&&!saved)await loadDashboard(false,{startup:true});
      else setTimeout(()=>{if(state.token)loadDashboard(false,{startup:false});},150);
      restoreHoldingsSummaryState();
      resetAutoRefreshClock();
    }
...
console.info('MyFinance v19.4.7 loaded — shared holdings recovery and professional diary enabled');
