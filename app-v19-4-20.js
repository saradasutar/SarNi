...
const APP_VERSION = '19.4.20';
const EXPECTED_BACKEND_VERSION = '3.17.7';
...
const APP_BUILD_ID = '20260826-19420-sticky1';
...
const MOBILE_DATA_CACHE_VERSION = '19.4.20-expenditure-categories-1';
const state = {
...
async function waitForAsyncResult(requestId,timeoutMs){
  ...
  throw new Error('The backend did not return a result. Confirm Backend v3.17.7 is deployed and config.js uses the same /exec URL.');
}
function apiViaAsyncPost(action,payload={},options={}){
  ...
  return new Promise((resolve,reject)=>{
    ...
    // Backend v3.17.7 replies directly to the hidden frame. Polling starts only
  ...
  try{
    ...
    if(data&&data.version){
      ...
      if(parts[0]<3||(parts[0]===3&&(parts[1]<17||(parts[1]===17&&(parts[2]||0)<2)))){
        showRuntimeWarning(`Backend v${state.backendVersion} is older than required v3.17.7. Deploy the included Code-v3.17.7.gs for expenditure subcategories.`);
      }
    }
  }catch(e){
...
async function retryStartupPortfolioLoad(){
  if(!state.token)return;
  setPortfolioStartupLoading(true,'Retrying Backend v3.17.7…',false);
...
console.info('MyFinance v19.4.20 loaded — editable expenditure categories and subcategories enabled');
