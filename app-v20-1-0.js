...
const APP_BUILD_ID = '20260901-20100-majorprojects1';
...
const MOBILE_DATA_CACHE_VERSION = '20.1.0-premium-command-centre-1';
const state = {
...
function toggleHoldingsSummaryFullscreen(force){const panel=els.holdingsSummaryPanel;if(!panel)return;const open=typeof force==='boolean'?force:!panel.classList.contains('summary-fullscreen');panel.classList.toggle('summary-fullscreen',open);document.body.classList.toggle('summary-fullscreen-open',open);if(open){setHoldingsMatrixCompare(true,false);panel.scrollIntoView({block:'start'});}els.holdingsSummaryFullscreenBtn?.setAttribute('aria-pressed',open?'true':'false');if(els.holdingsSummaryFullscreenBtn)els.holdingsSummaryFullscreenBtn.textContent=open?'× Close full screen':'⛶ Full-screen summary';}
...
function bindEvents(){
  ...
  safeOn(els.holdingsMatrixCompareBtn,'click',toggleHoldingsMatrixCompare);safeOn(els.holdingsSummaryFullscreenBtn,'click',()=>toggleHoldingsSummaryFullscreen());if(els.holdingsAdvancedTools){els.holdingsAdvancedTools.open=localStorage.getItem('sarni_holdings_tools_open')==='1';safeOn(els.holdingsAdvancedTools,'toggle',()=>localStorage.setItem('sarni_holdings_tools_open',els.holdingsAdvancedTools.open?'1':'0'));}
...
console.info('SarNi v20.1.0 loaded — premium responsive command centre ready');
