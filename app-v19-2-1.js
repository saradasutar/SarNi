...
function scrollDashboardHorizontal(direction){
  const target=state.hScrollTarget||activeHorizontalScrollTarget();
  if(!target)return;
  const step=Math.max(260,Math.min(720,target.clientWidth*.72));
  target.scrollBy({left:direction*step,behavior:'smooth'});
  queueHScrollUiRefresh();
  setTimeout(queueHScrollUiRefresh,180);
  setTimeout(queueHScrollUiRefresh,420);
}
...
console.info('MyFinance v19.2.1 loaded — smoother horizontal scrolling + pinnable sticky notes');
