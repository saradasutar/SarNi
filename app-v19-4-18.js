...
const APP_VERSION = '19.4.18';
const EXPECTED_BACKEND_VERSION = '3.17.6';
...
const APP_BUILD_ID = '20260825-19418-expense1';
...
const MOBILE_DATA_CACHE_VERSION = '19.4.18-expenditure-categories-1';
const state = {
...
async function waitForAsyncResult(requestId,timeoutMs){
  ...
  throw new Error('The backend did not return a result. Confirm Backend v3.17.6 is deployed and config.js uses the same /exec URL.');
}
function apiViaAsyncPost(action,payload={},options={}){
  ...
  return new Promise((resolve,reject)=>{
    ...
    // Backend v3.17.6 replies directly to the hidden frame. Polling starts only
  ...
  try{
    ...
    if(data&&data.version){
      ...
      if(parts[0]<3||(parts[0]===3&&(parts[1]<17||(parts[1]===17&&(parts[2]||0)<2)))){
        showRuntimeWarning(`Backend v${state.backendVersion} is older than required v3.17.6. Deploy the included Code-v3.17.6.gs for expenditure subcategories.`);
      }
    }
  }catch(e){
...
async function retryStartupPortfolioLoad(){
  if(!state.token)return;
  setPortfolioStartupLoading(true,'Retrying Backend v3.17.6…',false);
...
function renderActualExpenses(rows){
  ...
  els.actualExpenseBody.innerHTML=rows.map(x=>`<tr class="expense-row expense-row-${expenseCategoryColor(x.category)}">
    ...
    <td class="expense-notes">${escapeHtml(x.notes||'—')}</td><td class="expense-actions"><button class="small-button expense-edit-button" data-edit-expense="${escapeHtml(x.id)}">✎ Edit</button><button class="small-button danger" data-delete-expense="${escapeHtml(x.id)}">Delete</button></td>
  </tr>`).join('');
}
function renderPlannedExpenses(rows){
  ...
  els.plannedExpenseBody.innerHTML=rows.map(x=>{const planned=String(x.status||'PLANNED').toUpperCase()==='PLANNED';return `<tr class="expense-row expense-row-${expenseCategoryColor(x.category)}">
    ...
    <td class="expense-actions">${planned?`<button class="small-button success" data-mark-plan-paid="${escapeHtml(x.id)}">Mark paid</button>`:''}<button class="small-button expense-edit-button" data-edit-plan="${escapeHtml(x.id)}">✎ Edit</button><button class="small-button danger" data-delete-plan="${escapeHtml(x.id)}">Delete</button></td>
  </tr>`;}).join('');
}
function renderRegularExpenses(rows){
  ...
  els.regularExpenseBody.innerHTML=rows.map(x=>{const active=String(x.status||'ACTIVE').toUpperCase()==='ACTIVE';const toggle=active?'PAUSED':'ACTIVE';return `<tr class="expense-row expense-row-${expenseCategoryColor(x.category)}">
    ...
    <td class="expense-actions">${active?`<button class="small-button success" data-mark-regular-paid="${escapeHtml(x.id)}">Mark paid</button>`:''}<button class="small-button" data-toggle-regular="${escapeHtml(x.id)}" data-next-status="${toggle}">${active?'Pause':'Resume'}</button><button class="small-button expense-edit-button" data-edit-regular="${escapeHtml(x.id)}">✎ Edit</button><button class="small-button danger" data-delete-regular="${escapeHtml(x.id)}">Delete</button></td>
  </tr>`;}).join('');
}
...
console.info('MyFinance v19.4.18 loaded — editable expenditure categories and subcategories enabled');
