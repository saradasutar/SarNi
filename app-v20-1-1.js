...
const EXPECTED_BACKEND_VERSION = '3.17.9';
...
const APP_BUILD_ID = '20260901-20101-summarynav1';
...
const MOBILE_DATA_CACHE_VERSION = '20.1.1-premium-command-centre-1';
const state = {
...
async function waitForAsyncResult(requestId,timeoutMs){
  ...
  throw new Error('The backend did not return a result. Confirm Backend v3.17.9 is deployed and config.js uses the same /exec URL.');
}
function apiViaAsyncPost(action,payload={},options={}){
  ...
  return new Promise((resolve,reject)=>{
    ...
    // Backend v3.17.9 replies directly to the hidden frame. Polling starts only
  ...
  try{
    ...
    if(data&&data.version){
      ...
      if(parts[0]<3||(parts[0]===3&&(parts[1]<17||(parts[1]===17&&(parts[2]||0)<2)))){
        showRuntimeWarning(`Backend v${state.backendVersion} is older than required v3.17.9. Deploy the included Code-v3.17.9.gs for expenditure subcategories.`);
      }
    }
  }catch(e){
...
async function retryStartupPortfolioLoad(){
  if(!state.token)return;
  setPortfolioStartupLoading(true,'Retrying Backend v3.17.9…',false);
...
const HOLDINGS_MATRIX_COMPARE_KEY='sarni_holdings_matrix_compare_v1';
...
function setHoldingsMatrixCompare(compare=false,persist=true){
  const wrap=document.querySelector('.holding-matrix-wrap');
  wrap?.classList.toggle('is-compare-all',Boolean(compare));
  if(els.holdingsMatrixCompareBtn){
    els.holdingsMatrixCompareBtn.setAttribute('aria-pressed',compare?'true':'false');
    els.holdingsMatrixCompareBtn.textContent=compare?'Use focused view':'Compare all investors';
  }
  if(persist){try{localStorage.setItem(HOLDINGS_MATRIX_COMPARE_KEY,compare?'1':'0');}catch{}}
  requestAnimationFrame(updateHoldingSummaryNavigator);
  scheduleDashboardHScrollRefresh();
}
function toggleHoldingsMatrixCompare(){
  setHoldingsMatrixCompare(!document.querySelector('.holding-matrix-wrap')?.classList.contains('is-compare-all'));
}
function updateHoldingSummaryNavigator(){const wrap=document.querySelector('.holding-matrix-wrap'),nav=els.holdingSummaryNavigator;if(!wrap||!nav)return;const compare=wrap.classList.contains('is-compare-all');nav.classList.toggle('hidden',!compare);const max=Math.max(0,wrap.scrollWidth-wrap.clientWidth),pct=max?Math.round(wrap.scrollLeft/max*100):0;if(els.holdingSummaryScrollRange)els.holdingSummaryScrollRange.value=String(pct);if(els.holdingSummaryScrollLabel)els.holdingSummaryScrollLabel.textContent=`${pct}%`;if(els.holdingSummaryLeftBtn)els.holdingSummaryLeftBtn.disabled=wrap.scrollLeft<=1;if(els.holdingSummaryRightBtn)els.holdingSummaryRightBtn.disabled=wrap.scrollLeft>=max-1;}
function setHoldingSummaryScrollPercent(value){const wrap=document.querySelector('.holding-matrix-wrap');if(!wrap)return;const max=Math.max(0,wrap.scrollWidth-wrap.clientWidth);wrap.scrollTo({left:max*Math.max(0,Math.min(100,Number(value)||0))/100,behavior:'smooth'});}
function stepHoldingSummary(direction){const wrap=document.querySelector('.holding-matrix-wrap');if(!wrap)return;wrap.scrollBy({left:Number(direction)*Math.max(260,wrap.clientWidth*.72),behavior:'smooth'});}
function toggleHoldingsSummaryFullscreen(force){const panel=els.holdingsSummaryPanel;if(!panel)return;const open=typeof force==='boolean'?force:!panel.classList.contains('summary-fullscreen');panel.classList.toggle('summary-fullscreen',open);document.body.classList.toggle('summary-fullscreen-open',open);if(open){setHoldingsMatrixCompare(true,false);panel.scrollIntoView({block:'start'});}els.holdingsSummaryFullscreenBtn?.setAttribute('aria-pressed',open?'true':'false');if(els.holdingsSummaryFullscreenBtn)els.holdingsSummaryFullscreenBtn.textContent=open?'× Close full screen':'⛶ Full-screen summary';requestAnimationFrame(updateHoldingSummaryNavigator);}
function restoreHoldingsMatrixOwner(){
  ...
  let compare=false;
  try{compare=localStorage.getItem(HOLDINGS_MATRIX_COMPARE_KEY)==='1';}catch{}
  setHoldingsMatrixCompare(compare,false);
}
...
function bindEvents(){
  ...
  safeOn(els.holdingsMatrixCompareBtn,'click',toggleHoldingsMatrixCompare);safeOn(els.holdingsSummaryFullscreenBtn,'click',()=>toggleHoldingsSummaryFullscreen());safeOn(els.holdingSummaryScrollRange,'input',e=>setHoldingSummaryScrollPercent(e.target.value));safeOn(els.holdingSummaryLeftBtn,'click',()=>stepHoldingSummary(-1));safeOn(els.holdingSummaryRightBtn,'click',()=>stepHoldingSummary(1));safeOn(document.querySelector('.holding-matrix-wrap'),'scroll',updateHoldingSummaryNavigator,{passive:true});safeOn(window,'resize',updateHoldingSummaryNavigator);if(els.holdingsAdvancedTools){els.holdingsAdvancedTools.open=localStorage.getItem('sarni_holdings_tools_open')==='1';safeOn(els.holdingsAdvancedTools,'toggle',()=>localStorage.setItem('sarni_holdings_tools_open',els.holdingsAdvancedTools.open?'1':'0'));}
...
console.info('SarNi v20.1.1 loaded — premium responsive command centre ready');
